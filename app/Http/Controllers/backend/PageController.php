...
class PageController extends Controller
{
    ...
    {
        $pageshow = Post::where([['status', '!=', '0'], ['type', '=', 'page']])
        ->orderBy('created_at', 'desc')
        ->get();
        $page = Post::find($id);
        if ($page == null) {
        ...
        return view("backend.page.show", compact('title', 'page','pageshow'));
    }
    ...
    {
        $pageedit = Post::where([['status', '!=', '0'], ['type', '=', 'page']])
            ->orderBy('created_at', 'desc')
            ->get();
        $title = 'Sửa Trang Đơn';
        ...
        return view("backend.page.edit", compact('title', 'page', 'pageedit'));
    }
