...
class PostController extends Controller
{
    ...
    public function index()
    {
        ...
        return view("backend.post.index", compact('post', 'title', 'html_topic_id'));
    }
    ...
    {
        $postshow = Post::join('topic', 'post.topic_id', '=', 'topic.id')
        ->where('post.status', '!=', '0')
            ->orderBy('post.created_at', 'desc')
            ->select("post.*", "topic.name as topic_name")
            ->get();
        ...
        return view("backend.post.show", compact('title', 'post', 'postshow'));
    }
    ...
    {
        $postedit = Post::join('topic', 'post.topic_id', '=', 'topic.id')
        ->where('post.status', '!=', '0')
            ->orderBy('post.created_at', 'desc')
            ->select("post.*", "topic.name as topic_name")
            ->get();
        ...
        return view("backend.post.edit", compact('title', 'html_topic_id', 'post', 'postedit'));
    }
